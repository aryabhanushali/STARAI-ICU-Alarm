...
* The HiRID dataset is very large, so it’s read in chunks instead of all at once
* Data is resampled into 2-minute windows to reduce noise and match ICU monitoring systems
* Basic patient info like age and sex is added
...
The goal is to summarize each patient’s condition while still keeping meaningful clinical signals.
...
* Predictions from both models are later combined

---

### 4. Prediction + ensembling

* The 5 cross-validation models are combined using:

  * rank normalization
  * weighted averaging

* Final predictions are a blend of:

  * LightGBM (main model)
  * logistic regression (adds linear signal)
...
The model output is used to simulate different alert strategies, such as:
